...
"""
    proj_normalize_for_visualization(PJ_CONTEXT * ctx,
                                     const PJ * obj) -> PJ *
...
"""
    proj_context_set_database_path(PJ_CONTEXT * ctx,
                                   const char * dbPath,
...
"""
    proj_context_get_database_metadata(PJ_CONTEXT * ctx,
...
"""
    proj_context_guess_wkt_dialect(PJ_CONTEXT * ctx,
                                   const char * wkt) -> PJ_GUESSED_WKT_DIALECT
...
"""
    proj_create_from_wkt(PJ_CONTEXT * ctx,
                         const char * wkt,
...
"""
    proj_create_from_database(PJ_CONTEXT * ctx,
...
"""
    proj_uom_get_info_from_database(PJ_CONTEXT * ctx,
                                    const char * auth_name,
                                    const char * code,
                                    const char ** out_name,
                                    double * out_conv_factor,
                                    const char ** out_category) -> int
...
"""
    proj_create_from_name(PJ_CONTEXT * ctx,
                          const char * auth_name,
...
"""
    proj_get_non_deprecated(PJ_CONTEXT * ctx,
                            const PJ * obj) -> PJ_OBJ_LIST *
...
"""
    proj_as_proj_string(PJ_CONTEXT * ctx,
                        const PJ * obj,
...
"""
    proj_get_source_crs(PJ_CONTEXT * ctx,
                        const PJ * obj) -> PJ *
...
"""
    proj_get_target_crs(PJ_CONTEXT * ctx,
                        const PJ * obj) -> PJ *
...
"""
    proj_identify(PJ_CONTEXT * ctx,
                  const PJ * obj,
                  const char * auth_name,
                  const char *const * options,
                  int ** out_confidence) -> PJ_OBJ_LIST *
...
"""
    proj_get_codes_from_database(PJ_CONTEXT * ctx,
                                 const char * auth_name,
                                 PJ_TYPE type,
                                 int allow_deprecated) -> PROJ_STRING_LIST
...
"""
    proj_get_crs_info_list_from_database(PJ_CONTEXT * ctx,
                                         const char * auth_name,
                                         const PROJ_CRS_LIST_PARAMETERS * params,
                                         int * out_result_count) -> PROJ_CRS_INFO **
...
"""
    proj_create_operation_factory_context(PJ_CONTEXT * ctx,
                                          const char * authority) -> PJ_OPERATION_FACTORY_CONTEXT *
...
"""
    proj_operation_factory_context_set_desired_accuracy(PJ_CONTEXT * ctx,
...
"""
    proj_operation_factory_context_set_area_of_interest(PJ_CONTEXT * ctx,
                                                        PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                        double west_lon_degree,
                                                        double south_lat_degree,
                                                        double east_lon_degree,
                                                        double north_lat_degree) -> void
...
"""
    proj_operation_factory_context_set_crs_extent_use(PJ_CONTEXT * ctx,
                                                      PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                      PROJ_CRS_EXTENT_USE use) -> void
...
"""
    proj_operation_factory_context_set_spatial_criterion(PJ_CONTEXT * ctx,
                                                         PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                         PROJ_SPATIAL_CRITERION criterion) -> void
...
"""
    proj_operation_factory_context_set_grid_availability_use(PJ_CONTEXT * ctx,
                                                             PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                             PROJ_GRID_AVAILABILITY_USE use) -> void
...
"""
    proj_operation_factory_context_set_use_proj_alternative_grid_names(PJ_CONTEXT * ctx,
                                                                       PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                                       int usePROJNames) -> void
...
"""
    proj_operation_factory_context_set_allow_use_intermediate_crs(PJ_CONTEXT * ctx,
                                                                  PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                                  PROJ_INTERMEDIATE_CRS_USE use) -> void
...
"""
    proj_operation_factory_context_set_allowed_intermediate_crs(PJ_CONTEXT * ctx,
                                                                PJ_OPERATION_FACTORY_CONTEXT * factory_ctx,
                                                                const char *const * list_of_auth_name_codes) -> void
...
"""
    proj_create_operations(PJ_CONTEXT * ctx,
                           const PJ * source_crs,
                           const PJ * target_crs,
                           const PJ_OPERATION_FACTORY_CONTEXT * operationContext) -> PJ_OBJ_LIST *
...
"""
    proj_crs_get_geodetic_crs(PJ_CONTEXT * ctx,
                              const PJ * crs) -> PJ *
...
"""
    proj_crs_get_horizontal_datum(PJ_CONTEXT * ctx,
                                  const PJ * crs) -> PJ *
...
"""
    proj_crs_get_sub_crs(PJ_CONTEXT * ctx,
                         const PJ * crs,
                         int index) -> PJ *
...
"""
    proj_crs_get_datum(PJ_CONTEXT * ctx,
                       const PJ * crs) -> PJ *
...
"""
    proj_crs_get_coordinate_system(PJ_CONTEXT * ctx,
                                   const PJ * crs) -> PJ *
...
"""
    proj_cs_get_type(PJ_CONTEXT * ctx,
                     const PJ * cs) -> PJ_COORDINATE_SYSTEM_TYPE
...
"""
    proj_cs_get_axis_count(PJ_CONTEXT * ctx,
                           const PJ * cs) -> int
...
"""
    proj_cs_get_axis_info(PJ_CONTEXT * ctx,
                          const PJ * cs,
                          int index,
                          const char ** out_name,
...
"""
    proj_get_ellipsoid(PJ_CONTEXT * ctx,
                       const PJ * obj) -> PJ *
...
"""
    proj_ellipsoid_get_parameters(PJ_CONTEXT * ctx,
                                  const PJ * ellipsoid,
                                  double * out_semi_major_metre,
                                  double * out_semi_minor_metre,
                                  int * out_is_semi_minor_computed,
                                  double * out_inv_flattening) -> int
...
"""
    proj_get_prime_meridian(PJ_CONTEXT * ctx,
                            const PJ * obj) -> PJ *
...
"""
    proj_prime_meridian_get_parameters(PJ_CONTEXT * ctx,
                                       const PJ * prime_meridian,
                                       double * out_longitude,
                                       double * out_unit_conv_factor,
                                       const char ** out_unit_name) -> int
...
"""
    proj_crs_get_coordoperation(PJ_CONTEXT * ctx,
                                const PJ * crs) -> PJ *
...
"""
    proj_coordoperation_get_method_info(PJ_CONTEXT * ctx,
                                        const PJ * coordoperation,
                                        const char ** out_method_name,
                                        const char ** out_method_auth_name,
                                        const char ** out_method_code) -> int
...
"""
    proj_coordoperation_is_instantiable(PJ_CONTEXT * ctx,
                                        const PJ * coordoperation) -> int
...
"""
    proj_coordoperation_get_param_count(PJ_CONTEXT * ctx,
                                        const PJ * coordoperation) -> int
...
"""
    proj_coordoperation_get_param_index(PJ_CONTEXT * ctx,
                                        const PJ * coordoperation,
                                        const char * name) -> int
...
"""
    proj_coordoperation_get_param(PJ_CONTEXT * ctx,
                                  const PJ * coordoperation,
                                  int index,
                                  const char ** out_name,
                                  ...
                                  const char ** out_unit_category) -> int
...
"""
    proj_coordoperation_get_grid_used_count(PJ_CONTEXT * ctx,
                                            const PJ * coordoperation) -> int
...
"""
    proj_coordoperation_get_grid_used(PJ_CONTEXT * ctx,
                                      const PJ * coordoperation,
                                      int index,
...
"""
    proj_coordoperation_get_accuracy(PJ_CONTEXT * ctx,
                                     const PJ * coordoperation) -> double
...
"""
    proj_coordoperation_get_towgs84_values(PJ_CONTEXT * ctx,
                                           const PJ * coordoperation,
                                           double * out_values,
                                           int value_count,
                                           int emit_error_if_incompatible) -> int
